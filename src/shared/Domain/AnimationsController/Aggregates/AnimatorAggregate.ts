...
export class AnimatorAggregate {
    ...
    constructor(animatorProperties: AnimatorProperties) {
        ...
        if (animatorProperties.Animator) {
            this.animator = animatorProperties.Animator;
            return;
        }
        ...
        if (animatorProperties.AnimatorName) {
            const controller = !humanoid
                ? (this.character.FindFirstChildOfClass("AnimationController") as
                      | AnimationController
                      | undefined)
                : undefined;

            const parent = humanoid ?? controller;
            assert(
                parent,
                `[AnimatorAggregate] Neither Humanoid nor AnimationController found in "${this.character.Name}"`,
            );
        ...
        const controller = this.character.FindFirstChildOfClass("AnimationController") as
            | AnimationController
            | undefined;
        assert(
            controller,
        ...
        const animator = controller.FindFirstChildOfClass("Animator");
        assert(animator, "Animator was not found in AnimationController.");
        this.animator = animator;
    }
